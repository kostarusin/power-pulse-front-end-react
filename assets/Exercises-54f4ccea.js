...
`,Ze=h.li`
  margin-top: 32px;
`,R=({itemsPerPage:t,totalItems:s,currentPage:r,onPageChange:n})=>{const i=Math.ceil(s/t),o=a=>{a>=1&&a<=i&&n(a)},c=()=>{const a=[];for(let d=1;d<=i;d++)a.push(e.jsx(Ze,{className:`page-item ${d===r?"active":""}`,children:e.jsxs("button",{className:"page-link",onClick:()=>o(d),children:[d,d===r&&e.jsx("svg",{children:e.jsx("use",{href:`${I}#pagination-btn`})})]})},d));return a};return e.jsx("nav",{"aria-label":"Pagination",children:e.jsx("ul",{className:"pagination",children:c()})})};R.propTypes={itemsPerPage:k.number.isRequired,totalItems:k.number.isRequired,currentPage:k.number.isRequired,onPageChange:k.func.isRequired};const ae=({handleFilterClick:t,handleSetExName:s})=>{const r=C(),{bodyParts:n}=O();console.log(n.bodyParts),p.useEffect(()=>{r(L())},[r]);const[i,o]=p.useState(1),c=()=>{const x=window.innerWidth;return x>=768&&x<=1439?9:10},[a,d]=p.useState(c),l=()=>{d(c())};p.useEffect(()=>(window.addEventListener("resize",l),l(),()=>{window.removeEventListener("resize",l)}),[]);const m=x=>{o(x)};console.log();const u=i*a,g=u-a,y=n.bodyParts.slice(g,u);return e.jsxs(D,{children:[e.jsx(U,{children:y.map(x=>e.jsx(W,{exercisesItem:x,handleFilterClick:t,handleSetExName:s},x._id))}),a<n.bodyParts.length&&e.jsx(R,{itemsPerPage:a,totalItems:n.bodyParts.length,currentPage:i,onPageChange:m})]})};ae.propTypes={bodyParts:k.array};const et=h.div`
  padding-left: 20px;
  padding-right: 20px;
  margin-top: 10px;
